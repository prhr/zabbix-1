...
import json
from zabbix import Api, ApiException, Host, HostGroup
...
def api_session(auth=True):
    ...
    """
    with patch('zabbix.api.requests.session') as session:
        api = Api('http://zabbix', session)
...
def test_readonly1():
    ...
    with api_session() as api:
        ...
        with assert_raises(ApiException) as cm:
            grp.internal.val = 2
